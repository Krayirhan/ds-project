...
def cmd_preprocess(paths: Paths, cfg: ExperimentConfig) -> None:
    ...
    df = preprocess_basic(
        ...
        extra_blocked_cols=list(cfg.blocked_feature_cols) if cfg.blocked_feature_cols else None,
    )
    ...
    json_write(
        paths.reports_metrics / "data_lineage_preprocess.json",
        {
            ...
            "raw_dataset_sha256": sha256_file(str(raw_path)) if raw_path.exists() else None,
