...
def test_cmd_monitor_relative_paths_and_optional_branches(monkeypatch, tmp_path: Path):
    ...
    (run_dir / "reference_stats.json").write_text(
        json.dumps({"lead_time": {"mean": 10}}), encoding="utf-8"
    )
    (run_dir / "reference_correlations.json").write_text(
        json.dumps({"lead_time|adr": 0.2}), encoding="utf-8"
    )
    (run_dir / "feature_importance.json").write_text(
        json.dumps({"lead_time": 0.6, "adr": 0.4}), encoding="utf-8"
    )
    ...
    def _corr_drift(*args, **kwargs):
        ...
        return SimpleNamespace(
            n_drifted=1, drifted_pairs=[["lead_time", "adr"]], summary="corr"
        )
    ...
    monkeypatch.setattr(
        ...
        lambda _: {
            ...
            "categorical": ["arrival_date_month"],
        },
    )
    monkeypatch.setattr(
        monitor, "predict_with_policy", lambda **kwargs: (actions_cur, {"n_rows": 3})
    )
    monkeypatch.setattr(
        monitor,
    ...
    monkeypatch.setattr(
        monitor, "data_drift_report", lambda **kwargs: {"psi_scores": {}}
    )
    monkeypatch.setattr(
        monitor, "prediction_drift_report", lambda **kwargs: {"psi": 0.01}
    )
    monkeypatch.setattr(
        monitor, "outcome_monitoring_report", lambda **kwargs: {"profit": 100.0}
    )
