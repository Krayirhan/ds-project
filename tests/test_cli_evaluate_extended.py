...
def test_cmd_evaluate_no_policy_branch(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        cli_eval,
        "read_parquet",
        lambda *_args, **_kwargs: pd.DataFrame({"f": [1, 2], "is_canceled": [0, 1]}),
    )
    monkeypatch.setattr(
        cli_eval,
        "json_read",
        lambda p: (
            {"xgb": "models/xgb.joblib"}
            if p.name == "model_registry.json"
            else {"xgb": "sha"}
        ),
    )
    monkeypatch.setattr(cli_eval, "safe_load", lambda *_args, **_kwargs: _model())
    monkeypatch.setattr(
        cli_eval, "evaluate_binary_classifier", lambda *args, **kwargs: None
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: SimpleNamespace(
            best_threshold=0.5, best_profit=1.0, rows=[]
        ),
    )
    monkeypatch.setattr(
        cli_eval,
        ...
        lambda *args, **kwargs: SimpleNamespace(
            best_threshold=0.5, best_profit=float("nan"), rows=[]
        ),
    )
    monkeypatch.setattr(
        cli_eval,
        "pick_best_policy",
        lambda *args, **kwargs: {"status": "no_valid_candidate", "reason": "none"},
    )
...
def test_generate_explainability_import_error_and_missing_champion(
...
def test_generate_explainability_success_and_shap_exception(
    monkeypatch, tmp_path: Path
):
    paths = _paths(tmp_path)
    cfg = _cfg()
    ...
    tracker = SimpleNamespace(
        log_artifact=lambda *_args, **_kwargs: tracker_calls.__setitem__(
            "n", tracker_calls["n"] + 1
        )
    )
    test_df = pd.DataFrame({"f1": [1, 2], "is_canceled": [0, 1]})
    ...
    fake_explain.compute_permutation_importance = lambda *args, **kwargs: {
        "kind": "perm"
    }
    ...
    fake_explain.compute_shap_values = lambda *args, **kwargs: (_ for _ in ()).throw(
