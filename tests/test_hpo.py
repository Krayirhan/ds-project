...
class TestBuildEstimator:
    def test_builds_xgboost_classifier(self):
        ...
        try:
            est = _build_estimator("xgboost", {"n_estimators": 50, "learning_rate": 0.1, "max_depth": 3, "subsample": 0.8, "colsample_bytree": 0.8, "reg_lambda": 1.0, "reg_alpha": 0.01, "min_child_weight": 1}, seed=42)
    ...
    def test_builds_histgb_classifier(self):
        ...
        est = _build_estimator("histgb", {"learning_rate": 0.1, "max_depth": 4, "max_iter": 100, "min_samples_leaf": 10, "l2_regularization": 1.0}, seed=42)
        assert hasattr(est, "fit")
...
class TestRunHPO:
    ...
    def test_run_hpo_returns_hpo_result(self):
        ...
        mock_study.best_params = {"learning_rate": 0.1, "max_depth": 4, "max_iter": 100, "min_samples_leaf": 10, "l2_regularization": 1.0}
        mock_optuna.create_study.return_value = mock_study
        mock_optuna.logging = MagicMock()
        mock_optuna.logging.WARNING = 30

        with patch.dict("sys.modules", {"optuna": mock_optuna}):
            result = run_hpo(df, "is_canceled", seed=42, cv_folds=2, n_trials=3, model_type="histgb")
    ...
    def test_run_hpo_calls_optimize_with_n_trials(self):
        ...
        mock_study.best_params = {"learning_rate": 0.05, "max_depth": 3, "max_iter": 200, "min_samples_leaf": 20, "l2_regularization": 0.5}
        mock_optuna.create_study.return_value = mock_study
        mock_optuna.logging = MagicMock()
        mock_optuna.logging.WARNING = 30

        with patch.dict("sys.modules", {"optuna": mock_optuna}):
            run_hpo(df, "is_canceled", seed=42, cv_folds=2, n_trials=7, model_type="histgb")
    ...
    def test_run_hpo_detects_model_type_automatically(self):
        ...
        mock_study.best_params = {"learning_rate": 0.1, "max_depth": 4, "max_iter": 100, "min_samples_leaf": 5, "l2_regularization": 1.0}
        mock_optuna.create_study.return_value = mock_study
        mock_optuna.logging = MagicMock()
        mock_optuna.logging.WARNING = 30
        ...
        with (
            patch.dict("sys.modules", {"optuna": mock_optuna, "xgboost": None, "lightgbm": None}),
        ):
